...
// The API key must be sourced from process.env.API_KEY and is assumed to be available.
const ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });

export const generateDescription = async (productName: string): Promise<string> => {
  // FIX: Removed the manual API key check as its presence is a prerequisite.
  try {
